...
module.exports = function(options, callback) {
							...
							if (err || !stats.isFile()) {
								...
								readFile(file, function(err, content, isbinary) {
									if (err) {
										...
										var processedContent = isbinary ? content : content.replace(/%package\.([\w._-]+)%/g, function(match, key) {
										...
										writeDestFile(file, isbinary, processedContent, cb);
									}
								});
							}
						});
	...
	function writeDestFile(sourceFile, isbinary, processedContent, callback) {
		resolveDestFile(sourceFile, function(destFile) {
			mkdirp(path.dirname(sourceFile), function (err) {
				if (err) {
					callback(err);
				} else if (isbinary) {
