...
module.exports = function(options, callback) {
	...
	if (typeof(options.varPattern) === "string") {
		options.variablePattern = new RegExp(options.varPattern, "g");
	} else if (typeof(options.varPattern) === "undefined" || !options.varPattern) {
	...
	function log(level, message) {
		if (LOG_LEVELS.indexOf(level) <= loglevel) {
			if (console[level]) {
				console[level]((level + "  ").substring(0, 6) + message);
			} else {
				console.log((level + "  ").substring(0, 6) + message);
			}
		}
	}
								...
								readFile(file, function(err, content, isbinary) {
									if (err) {
										...
										if (isbinary) {
											...
											processedContent = content.replace(options.varPattern, function(match, key) {
